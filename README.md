...
> Apache-2.0 ライセンスの**無保証**ソフトウェアで、**サポート対象外**です。本番利用の際は内容を理解した上で、自己責任でご利用ください。

## これで学べること / デモすること

- WordPress の固定ページ・投稿に、ショートコード `[qtf_form]` でフォームを埋め込む方法
...
1. `questetra-trigger-form/` フォルダを `wp-content/plugins/` に置く（または ZIP 化して管理画面からアップロード）。
2. 管理画面 → プラグイン → 「**Questetra Trigger Form (Reference)**」を有効化。
...
`questetra-trigger-form/questetra-trigger-form.php` 冒頭の **設定ブロックだけ**を編集します（その下のロジック本体は編集不要）。

- `QTF_ENDPOINT_URL` … 対象の開始イベントの起動 URL（末尾の `?key=` は付けない）
- `QTF_API_KEY` … API Key。**プレースホルダのままにせず、自分のキーに置き換えてください。**
  公開リポジトリ等に載せる場合は直書きを避け、`wp-config.php` に
  `define( 'QTF_API_KEY', 'xxxx' );` と定義する形を推奨します。
- `QTF_FIELDS` … フォーム項目（＝受信パラメータ）の定義。増減で任意の構成に対応:
  ...
  | キー | 説明 |
  |------|------|
  | `param` | Questetra の受信パラメータ名（半角英数字・`_` のみ。`action` / `qtf_*` と重複させない） |
  | `label` | 画面に表示するラベル |
...
2. ハンドラが入力を検証し、`QTF_FIELDS` を基に multipart/form-data を組み立てて、サーバから Questetra の開始イベントへ POST します（ファイルは `param` 名で繰り返し送信）。
...
cd example-docker
docker compose up -d
# http://localhost:8080 で初期セットアップ → プラグイン有効化 → 固定ページに [qtf_form]
```

※ 送信を成功させるには、プラグインの `QTF_API_KEY` / `QTF_ENDPOINT_URL` / `QTF_FIELDS` を自分の開始イベントに合わせて設定してください。
